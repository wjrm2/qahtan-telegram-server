...
services = [name.strip() for names in groups.values() for name in names.split(",")]
assert len(services) == 170, len(services)
...
for index, category in enumerate(groups):
    token = f"c{index:02d}"
    assert len(f"svc_cat:{token}".encode("utf-8")) <= 64
    assert len(f"svc_page:9:{token}".encode("utf-8")) <= 64
